...
#
# Returns the base OTU table such that any OTUs with less than minPositives number of non-zero values are removed
#
def filterOTUByMinPositives(base, minPositives):
    newOTUTable = np.array(base)

    dimens = newOTUTable.shape
    ...
    while j >= OTU_START_COL:
        ...
        i = 1
        while i < dimens[0]:
            if float(newOTUTable[i,j]) != 0:
                nonZeroCount += 1
            i += 1
        if nonZeroCount < minPositives:
            newOTUTable = np.delete(newOTUTable, j, 1)
        j -= 1
    return newOTUTable.tolist()
...
def getTtest(statsAbundances):
    ...
    statistics = []
    abundanceKeys = list(statsAbundances.keys())
